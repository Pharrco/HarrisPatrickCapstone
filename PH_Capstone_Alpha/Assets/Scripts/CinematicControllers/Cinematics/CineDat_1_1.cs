...
public class CineDat_1_1 : CinematicBase
{
    ...
    private void Awake()
    {
		...
		Cinematic_event_queue.Enqueue(new PauseEvent());
		Cinematic_event_queue.Enqueue(new ClearDialogueEvent());
		Cinematic_event_queue.Enqueue(new DialogueEvent(roommate_profile, "YOU", "You really shouldn't.", false));
		Cinematic_event_queue.Enqueue(new PauseEvent());
		Cinematic_event_queue.Enqueue(new ClearDialogueEvent());
		...
		Cinematic_event_queue.Enqueue(new PauseEvent());
		Cinematic_event_queue.Enqueue(new ClearDialogueEvent());
		Cinematic_event_queue.Enqueue(new DialogueEvent(roommate_profile, "CRAZY (RICH) OLD LADY", "My butler spotted the little dear sneaking into the condemned home on the other side of yours.", false));
		Cinematic_event_queue.Enqueue(new PauseEvent());
		Cinematic_event_queue.Enqueue(new CharRotateEvent(hero_char, new Vector2(4, 1)));
		Cinematic_event_queue.Enqueue(new TimePauseEvent(0.5f));
		Cinematic_event_queue.Enqueue(new ShowImageEvent(image_mansionSpooky));
		Cinematic_event_queue.Enqueue(new TimePauseEvent(1f));
		Cinematic_event_queue.Enqueue(new DialogueEvent(roommate_profile, "YOU", "Oh, you mean the other terrifying castle I've never seen before. My memory must suck, because I can't even remember what I'm tripping on right now.", false));
		Cinematic_event_queue.Enqueue(new PauseEvent());
		Cinematic_event_queue.Enqueue(new HideImageEvent());
		Cinematic_event_queue.Enqueue(new CharRotateEvent(hero_char, new Vector2(3, 4)));
		Cinematic_event_queue.Enqueue(new DialogueEvent(roommate_profile, "CRAZY (RICH) OLD LADY", "I'm sure the poor dear is so terrified, being up in that scary place all alone.", false));
		Cinematic_event_queue.Enqueue(new PauseEvent());
		Cinematic_event_queue.Enqueue(new ClearDialogueEvent());
		Cinematic_event_queue.Enqueue(new DialogueEvent(roommate_profile, "YOU", "At this point I'm just assuming you can't hear a single word I'm saying.", false));
		Cinematic_event_queue.Enqueue(new PauseEvent());
		Cinematic_event_queue.Enqueue(new ClearDialogueEvent());
		Cinematic_event_queue.Enqueue(new DialogueEvent(roommate_profile, "CRAZY (RICH) OLD LADY", "I would be so grateful if you would go and rescue the poor dear and bring him home. I would do it myself, but my poor old heart just couldn't take searching that big house.", false));
		Cinematic_event_queue.Enqueue(new PauseEvent());
		Cinematic_event_queue.Enqueue(new ClearDialogueEvent());
		Cinematic_event_queue.Enqueue(new DialogueEvent(roommate_profile, "YOU", "I'm going to go with \"Hell No\". Have your butler go take a look around. What could possibly make you think I would risk my life for...", false));
		Cinematic_event_queue.Enqueue(new PauseEvent());
		Cinematic_event_queue.Enqueue(new ClearDialogueEvent());
		Cinematic_event_queue.Enqueue(new DialogueEvent(roommate_profile, "CRAZY (RICH) OLD LADY", "There will obviously be a very generous reward when my Little Angel is returned.", false));
		Cinematic_event_queue.Enqueue(new PauseEvent());
		Cinematic_event_queue.Enqueue(new ClearDialogueEvent());
		Cinematic_event_queue.Enqueue(new ActivateObjEvent(cash_effect));
		Cinematic_event_queue.Enqueue(new DialogueEvent(roommate_profile, "YOU", "$uddenly, the $afety and wellne$$ of your preciou$ trea$ure i$ my mo$t pre$$ing concern.", false));
		Cinematic_event_queue.Enqueue(new PauseEvent());
		Cinematic_event_queue.Enqueue(new ClearDialogueEvent());
	}
