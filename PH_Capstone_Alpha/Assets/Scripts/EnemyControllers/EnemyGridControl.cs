...
public class EnemyGridControl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (PhaseController.GetCurrPhase() == GamePhase.PlayerResult)
        {
			...
			for (int x = 0; x < BuildBoard.GetArrayHeight(); x++)
			{
				for (int y = 0; y < BuildBoard.GetArrayWidth(); y++)
				{
					if ((enemy_array[x,y] != null) && (enemy_array[x, y].GetComponent<BaseEnemyController>() != null))
					{
						enemy_array[x, y].GetComponent<BaseEnemyController>().LightEffect(LightEffectControl.GetLightPoint(x, y));
					}
				}
			}
